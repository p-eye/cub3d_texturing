...
직접 대입해보면 다음과 같다
|screenHeihgt|240|240|240|240|240|
|------|---|---|---|---|---|
|y|0|60|120|180|239|
...
따라서 while문 안에서 buf[y][x]와 buf[h - 1 - y][x]로 위아래를 한번에 처리해줄 수 있기 때문에 반복문을 절반만 돌린다. <br />
...
while (y < screenHeight / 2)
{
    ...
    while (x < screenWidth)
    {
        ...
        buf[h - 1 - y][x] = data[tex_point]; // floor
        obj_x += step_x;
        obj_y += step_y;
        x++;
    }
